    ...
    try {
        ...
        return (
            <div className="p-8 text-center text-zinc-400">
                Please complete your profile to view the dashboard.
            </div>
        );
    }

    const { metrics: dataMetrics, feed, tasks, user } = dashboardData;

    const metrics = [
        { label: "Total Leads", value: dataMetrics.totalLeads.toString(), change: "+12.5%", trend: "up", icon: Users, color: "text-blue-400", bg: "bg-blue-400/10" },
        { label: "Response Rate", value: dataMetrics.responseRate, change: "+2.1%", trend: "up", icon: Zap, color: "text-amber-400", bg: "bg-amber-400/10" },
        { label: "Meetings Booked", value: dataMetrics.meetingsBooked.toString(), change: "+8", trend: "up", icon: CalendarCheck, color: "text-emerald-400", bg: "bg-emerald-400/10" },
        { label: "Avg. Response Time", value: dataMetrics.avgResponseTime, change: "-12s", trend: "down", icon: Clock, color: "text-purple-400", bg: "bg-purple-400/10" },
    ];
    ...
    return (
        <div className="space-y-8 animate-fade">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-semibold text-white tracking-tight">Good afternoon, {user.firstName || "User"}</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="border border-white/5 rounded-xl bg-white/[0.01] overflow-hidden min-h-[100px]">
                        {feed.length === 0 ? (
                            <div className="p-4 text-sm text-zinc-500 text-center">No recent activities.</div>
                        ) : feed.map((item) => (
                            <div key={item.id} className="flex items-center gap-4 p-4 border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors group">
                                ...
                                <div>
                                    <span className={`px-2 py-1 rounded text-[10px] font-medium border ${item.status === 'Replied' ? 'border-emerald-500/20 text-emerald-400' :
                                        item.status === 'Hot' ? 'border-red-500/20 text-red-400' :
                ...
                <div className="space-y-6">
                    ...
                    <div className="p-5 rounded-xl border border-indigo-500/20 bg-gradient-to-b from-indigo-500/10 to-transparent relative overflow-hidden">
                        ...
                        <p className="text-xs text-indigo-200/80 leading-relaxed">
                            You have <span className="font-semibold text-white">0 urgent leads</span> waiting for approval.
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {tasks.length === 0 ? (
                                <div className="text-xs text-zinc-500 italic p-2 border border-dashed border-white/10 rounded-lg">No pending tasks. You're all caught up!</div>
                            ) : tasks.map(task => (
                                <div key={task.id} className="p-3 rounded-lg border border-white/5 bg-zinc-900/50 hover:border-zinc-700 transition-colors flex items-start gap-3 group">
                                    ...
                                    <div className="flex-1">
                                        <div className="text-xs text-zinc-300 group-hover:text-white transition-colors leading-snug">{task.title}</div>
                                        <div className="mt-1 flex items-center gap-2">
                                            ...
                                            {task.dueDate && (
                                                <span className="text-[10px] text-zinc-600 flex items-center gap-1">
                                                    <Clock3 className="w-3 h-3" /> {formatDistanceToNow(new Date(task.dueDate), { addSuffix: true })}
                                                </span>
                                            )}
                                        </div>
                                    </div>
